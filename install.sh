...
function ssl-certs() {
  ...
  local services="db py-srv"
  local target_dir="cert"


  for s in `echo $services | awk '{print $0;}'`; do
    if [[ $flag == 1 ]]; then
      #statements
      create-certs $docker_img_tag $docker_img_name $s $target_dir
    else
      remove-certs $docker_img_tag $s $target_dir
    fi
  done
}
function start-up(){
